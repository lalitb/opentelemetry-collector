...
# Build collector if not already built
if [ ! -f "../bin/otelcorecol_$(go env GOOS)_$(go env GOARCH)" ]; then
    echo ""
    echo "Building collector with GigWarm exporter..."
    cd ..
...
COLLECTOR_BIN="../bin/otelcorecol_$(go env GOOS)_$(go env GOARCH)"
